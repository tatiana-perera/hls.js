...
class Part extends BaseSegment {
  ...
  get loaded() {
    ...
    return !!(elementaryStreams.audio || elementaryStreams.video || elementaryStreams.audiovideo || this.fragment.type === PlaylistLevelType.SUBTITLE && this.stats.loaded);
  }
}
...
function getSampleData(data, initData, chunkMeta, logger) {
  const tracks = {};
  ...
  for (let mi = 0; mi < moofs.length; mi++) {
    ...
    for (let i = 0; i < trafs.length; i++) {
      ...
      const defaultSampleSize = (trackDefault == null ? void 0 : trackDefault.sampleSize) || 0;
...
class AbrController extends Logger {
    ...
    this._abandonRulesCheck = levelLoaded => {
      ...
      Time to underbuffer: ${bufferStarvationDelay.toFixed(3)} s`);
          ...
          if (nextLoadLevel > minAutoLevel) {
            ...
            this.firstAutoFloor = Math.min(lowestSwitchLevel, this.hls.firstLevel);
  ...
  findBestLevel(currentBw, minAutoLevel, maxAutoLevel, bufferStarvationDelay, maxStarvationDelay, bwFactor, bwUpFactor) {
    ...
    for (let i = maxAutoLevel; i >= minAutoLevel; i--) {
      ...
      i === lastLoadedFragLevel || levelInfo.loadError === 0 && levelInfo.fragmentError === 0 || isPenaltyExpired(levelInfo, this.hls.config.errorPenaltyExpireMs)) && (
...
class ErrorController extends Logger {
  ...
  getLevelSwitchAction(data, levelIndex) {
    ...
    if (level) {
      ...
      const isAudioCodecError = fragErrorType === PlaylistLevelType.AUDIO && errorDetails === ErrorDetails.FRAG_PARSING_ERROR || data.sourceBufferName === 'audio' && isCodecRelated(errorDetails);
      const findAudioCodecAlternate = isAudioCodecError && levels.some(({
        audioCodec
      }) => level.audioCodec !== audioCodec);
      // Find alternate video codec if available on video codec error
      const isVideoCodecError = data.sourceBufferName === 'video' && isCodecRelated(errorDetails);
  ...
  onErrorOut(event, data) {
    ...
    switch ((_data$errorAction = data.errorAction) == null ? void 0 : _data$errorAction.action) {
      ...
      case NetworkErrorAction.SendAlternateToPenaltyBox:
        ...
        if (!data.errorAction.resolved && data.details !== ErrorDetails.FRAG_GAP && data.details !== ErrorDetails.PLAYLIST_UNCHANGED_ERROR) {
          data.fatal = true;
        }
        break;
...
function isCodecRelated(errorDetails) {
  return errorDetails === ErrorDetails.BUFFER_ADD_CODEC_ERROR || errorDetails === ErrorDetails.BUFFER_APPEND_ERROR || errorDetails === ErrorDetails.MEDIA_SOURCE_REQUIRES_RESET;
}
...
class Decrypter {
  ...
  decrypt(data, key, iv, aesMode, plainTextLength = 0) {
...
class ChunkMetadata {
  constructor(level, sn, id, size = 0, part = -1, partial = false, duration, iframe) {
    this.level = void 0;
    this.sn = void 0;
    this.part = void 0;
    this.id = void 0;
...
class BaseStreamController extends TaskLoop {
  ...
  _loadInitSegment(fragment, level) {
    this._doFragLoad(fragment, level).then(data => {
      ...
      if (payload && payload.byteLength > 0 && decryptData != null && decryptData.key && decryptData.iv && isFullSegmentEncryption(decryptData.method)) {
        ...
        const byteRange = frag.byteRange;
        const plainTextLength = byteRange.length ? byteRange[1] - byteRange[0] : 0;
        return this.decrypter.decrypt(new Uint8Array(payload), decryptData.key.buffer, decryptData.iv.buffer, getAesModeFromFullSegmentMethod(decryptData.method), plainTextLength).catch(err => {
          hls.trigger(Events.ERROR, {
            type: ErrorTypes.MEDIA_ERROR,
  ...
  _handleFragmentLoadComplete(fragLoadedEndData) {
    ...
    const chunkMeta = new ChunkMetadata(frag.level, frag.sn, frag.stats.chunkCount + 1, 0, part ? part.index : -1, !complete, frag.duration, this.iframesOnly);
  ...
  _doFragLoad(frag, level, targetBufferTime = null, progressCallback) {
    ...
    if (dataOnProgress && keyLoadingPromise) {
      result = keyLoadingPromise.then(keyLoadedData => {
        ...
        return this.fragmentLoader.load(frag, this.iframesOnly, progressCallback);
      }).catch(error => this.handleFragLoadError(error));
    } else {
      // load unencrypted fragment data with progress event,
      // or handle fragment result after key and fragment are finished loading
      result = Promise.all([this.fragmentLoader.load(frag, this.iframesOnly, dataOnProgress ? progressCallback : undefined), keyLoadingPromise]).then(([fragLoadedData]) => {
    ...
    if (levelDetails.live) {
      ...
      if (!levelDetails.PTSKnown && !this.startFragRequested && this.startPosition === -1 || pos < playlistStart) {
        ...
        if (this.startPosition != startPosition || this.nextLoadPosition != startPosition) {
          this.log(`Setting startPosition to ${startPosition} ${mainStart === -1 ? '' : `(from ${configValue}) `}based on ${reason}. live edge: ${liveSyncPosition} live frag start: ${fragStart} playlist start: ${playlistStart} buffer pos: ${pos}`);
  ...
  */
  ...
  updateLevelTiming(frag, part, level, partial) {
    ...
    const parsed = Object.keys(frag.elementaryStreams).reduce((result, type) => {
      ...
      if (info) {
        ...
        const drift = partial ? 0 : updateFragPTSDTS(details, frag, info.startPTS, info.endPTS, info.startDTS, info.endDTS, this.iframesOnly, this);
  ...
  calculateOptimalSwitchPoint(nextLevel, bufferInfo) {
    ...
    if (media && !media.paused && levels) {
      ...
      const fragDuration = levelDetails && (this.loadingParts ? levelDetails.partTarget : levelDetails.averagetargetduration) || ((_this$fragCurrent2 = this.fragCurrent) == null ? void 0 : _this$fragCurrent2.duration) || 6;
...
transfer tracks: ${stringify(transferredTracks, (key, value) => key === 'initSegment' ? undefined : value)}}`);
  ...
  onBufferAppending(event, eventData) {
    ...
    if (videoSb && sn !== 'initSegment' && offset !== undefined) {
      ...
      if (type === 'audio' && parent !== 'main' && !(videoTrack.ending || videoTrack.ended)) {
        const pStart = partOrFrag.start;
        const pTime = pStart + partOrFrag.duration * 0.05;
    ...
    const operation = {
      ...
      onError: error => {
        ...
        if (error.name === TRACK_REMOVED_ERROR_NAME && this.sourceBufferCount === 0) {
  ...
  trimBuffers(frontBufferFlushThreshold, backBufferLength) {
    const {
      hls,
      details,
      media
  ...
  isAudioBlocked() {
    var _this$currentOp;
    return ((_this$currentOp = this.currentOp('audio')) == null ? void 0 : _this$currentOp.label) === 'block-audio';
  }
  isAudioBlocking() {
    if (this.operationQueue) {
  ...
  generateIS(audioTrack, videoTrack, timeOffset, accurateTimeOffset) {
    ...
    if (audioTrack.config && audioSamples.length) {
      ...
      if (computePTSDTS) {
        ...
        if (!_initPTS || timescale !== _initPTS.timescale || timeOffset === 0 && initBaseTimestamp < _initPTS.baseTime) {
    ...
    if (videoTrack.sps && videoTrack.pps && videoSamples.length) {
      ...
      if (computePTSDTS) {
        ...
        if (!_initPTS || timescale !== _initPTS.timescale || isFiniteNumber(initPTS) || timeOffset === 0 && videoInitPTS < _initPTS.baseTime) {
...
class Transmuxer {
  ...
  push(data, decryptdata, chunkMeta, state) {
    ...
    if (keyData && isFullSegmentEncryption(keyData.method)) {
      ...
      const plainTextLength = chunkMeta.size !== data.byteLength && chunkMeta.iframe ? chunkMeta.size : 0;
      this.asyncResult = true;
      this.decryptionPromise = decrypter.decrypt(uintData, keyData.key.buffer, keyData.iv.buffer, aesMode, plainTextLength).then(decryptedData => {
  ...
  flushRemux(transmuxResults, demuxResult, chunkMeta) {
    ...
    const remuxResult = this.remuxer.remux(audioTrack, videoTrack, id3Track, textTrack, timeOffset, accurateTimeOffset, true, this.id, chunkMeta);
  ...
  transmuxUnencrypted(data, timeOffset, accurateTimeOffset, chunkMeta) {
    const {
      ...
      id3Track,
      textTrack
    } = this.demuxer.demux(data, timeOffset, chunkMeta, false, !this.config.progressive);
    const remuxResult = this.remuxer.remux(audioTrack, videoTrack, id3Track, textTrack, timeOffset, accurateTimeOffset, false, this.id, chunkMeta);
    return {
      remuxResult,
      chunkMeta
    };
  ...
  transmuxSampleAes(data, decryptData, timeOffset, accurateTimeOffset, chunkMeta) {
    return this.demuxer.demuxSampleAes(data, decryptData, timeOffset, chunkMeta).then(demuxResult => {
      const remuxResult = this.remuxer.remux(demuxResult.audioTrack, demuxResult.videoTrack, demuxResult.id3Track, demuxResult.textTrack, timeOffset, accurateTimeOffset, false, this.id, chunkMeta);
      return {
        remuxResult,
        chunkMeta
      };
...
class StreamController extends BaseStreamController {
  ...
  */
  ...
  flushMainBuffer(startOffset, endOffset) {
    super.flushMainBuffer(startOffset, endOffset, this.altAudio === AlternateAudio.SWITCHED ? 'video' : null);
  }
  onMediaAttached(event, data) {
  ...
  _handleFragmentLoadProgress(data) {
    ...
    const byteRange = frag.byteRange;
    const chunkMeta = new ChunkMetadata(frag.level, frag.sn, frag.stats.chunkCount, byteRange.length ? byteRange[1] - byteRange[0] : payload.byteLength, partIndex, partial, frag.duration, this.iframesOnly);
      ...
      if (!frag.decryptdata.keyId && (_frag$initSegment = frag.initSegment) != null && _frag$initSegment.data) {
        ...
        if (keyIds.length) {
          ...
          if (keyId.some(b => b !== 0)) {
            ...
            this.log(`Patching empty keyId with ${arrayToHex(keyId)} keyUri: ${keyUri}`);
...
class Hls {
  ...
  constructor(userConfig = {}) {
    ...
    const coreComponents = [abrController, bufferController];
    if (gapController) {
   ...
   */
  get nextAudioTrack() {
    var _this$audioStreamCont, _this$audioStreamCont2;
    return (_this$audioStreamCont = (_this$audioStreamCont2 = this.audioStreamController) == null ? void 0 : _this$audioStreamCont2.nextAudioTrack) != null ? _this$audioStreamCont : -1;
  }
